...
public class PeriodicTableView extends View {
    ...
    private void drawSpace(Canvas c){
        float w=getWidth(),h=getHeight();p.setStyle(Paint.Style.FILL);
        // star cluster / nebula
        p.setShader(new RadialGradient(w*.22f,h*.28f,w*.34f,new int[]{Color.argb(80,73,35,129),Color.argb(40,31,78,132),Color.TRANSPARENT},null,Shader.TileMode.CLAMP));c.drawRect(0,0,w,h,p);p.setShader(null);
        p.setShader(new RadialGradient(w*.31f,h*.39f,w*.24f,new int[]{Color.argb(42,212,97,160),Color.argb(18,70,120,190),Color.TRANSPARENT},null,Shader.TileMode.CLAMP));c.drawRect(0,0,w,h,p);p.setShader(null);
        for(int i=0;i<180;i++){float x=(i*157%1009)/1009f*w,y=(i*271%1013)/1013f*h;float tw=.55f+.45f*(float)Math.sin(Math.toRadians(phase*2+i*23));p.setColor(Color.argb((int)(45+150*tw),160+(i%3)*25,180+(i%2)*35,255));c.drawCircle(x,y,.7f+(i%4)*.42f,p);}
        drawSaturn(c,w*.83f,h*.22f,Math.min(w,h)*.105f);
    }
    ...
    private void drawTable(Canvas c){
        ...
        p.setShadowLayer(28,16,20,Color.BLACK);p.setColor(Color.rgb(25,28,38));p.setStyle(Paint.Style.FILL);c.drawRoundRect(28,-2,1502,742,28,28,p);p.clearShadowLayer();
        ...
        p.setColor(Color.rgb(226,201,121));p.setTextSize(22);p.setTextAlign(Paint.Align.LEFT);c.drawText("PERIODIC  •  INTERACTIVE ELEMENT TABLE",63,718,p);
    ...
    private void drawTile(Canvas c,Element e,RectF r,boolean active){
        ...
        int text=Color.rgb(18,20,24);p.setShadowLayer(1.3f,0,1,Color.argb(150,255,255,255));p.setColor(text);p.setTextAlign(Paint.Align.LEFT);p.setTextSize(11);c.drawText(String.valueOf(e.number),top.left+7,top.top+14,p);p.setTextAlign(Paint.Align.CENTER);p.setTextSize(27);c.drawText(e.symbol,top.centerX(),top.top+40,p);p.setTextSize(8.8f);c.drawText(e.name,top.centerX(),top.top+53,p);p.setTextSize(8.2f);c.drawText(e.mass,top.centerX(),top.top+64,p);p.clearShadowLayer();
    }
